  ...
  if (event.keyCode == 46) 
    {
        ...
        encyclopaedia.filtergrid1(comp_code, house, rashi, planet, textname, book, page0,ext, extra1, exec_gridcall)
        ...
        if (dsexec.Tables[0].Rows.length > 0) {
            ...
            for (i = 0; i < dsexec.Tables[0].Rows.length; ++i) {
                ...
                buf2 += "</td>" 
                 ...
                 buf2 += "<td  style='border:0px solid #7DAAE3;'  align='left' >"
                 buf2 += "<Button  style='width:65px;color:green;font-weight: bold;' align='left' class='dropdown_large_corr'; Text='Publish' value='Publish' AutoPostBack='true' id='chk_" + i + "' onClick='javascript:return PublishNatalSignificators(this.id);' >Publish</Button>"
                 buf2 += "</td>"
                 }
                 else
                 {
                  buf2 += "<td    style='border:0px solid #7DAAE3;'  align='left' >"
                 buf2 += "<Button  style='width:65px;' align='left' class='dropdown_large_corr'; Text='Publish' value='Publish' AutoPostBack='true' id='chk_" + i + "' onClick='javascript:return PublishNatalSignificators(this.id);' >Publish</Button>"
                 buf2 += "</td>"
                 }
                 if (dsexec.Tables[0].Rows[i]['STATUS'] == "P") 
                 {
                 buf2 += "<td   style='border:0px solid #7DAAE3;'  align='left' >"
                 buf2 += "<Button  style='width:75px;color:red;font-weight: bold;' align='left' class='dropdown_large_corr'; Text='Unpublish' value='Unpublish' AutoPostBack='true' id='chk_" + i + "' onClick='javascript:return UnpublishNatalSignificators(this.id);' >Unpublish</Button>"
                 buf2 += "</td>"
                 }
                 else
                 {
                  buf2 += "<td  style='border:0px solid #7DAAE3;'  align='left' >"
                 buf2 += "<Button  style='width:75px;' align='left' class='dropdown_large_corr'; Text='Unpublish' value='Unpublish' AutoPostBack='true' id='chk_" + i + "' onClick='javascript:return UnpublishNatalSignificators(this.id);' >Unpublish</Button>"
                 buf2 += "</td>"
                 }
...
function showext12(id) {
    ...
    document.getElementById('Hidden6').value = extenvalue
     ...
     document.getElementById('Hidden7').value = exten
   var res = encyclopaedia.showextvalue(exten,extenvalue,exec_showextvalue);
...
//         buf2 += "</tr>"
 ...
 function significatorinsert() {
    ...
    //var newTemp = mystring.replace(/'/g, '"');
     ...
     encyclopaedia.save1(casecode, casename, caseplanet, casebook, casepage0, casehouse, caseplanet1, caserashi, caseext)
     alert('data saved successfully');
...
function PublishNatalSignificators(id) {
    var update = id.split('_')
    var update1 = update[1];
    var key = dsexec.Tables[0].Rows[update1].KEY_STRINGS
    mystring = document.getElementById("key_" + update1).value;
    ...
    var res = encyclopaedia.PublishUnpublishSignificators(key, key1,housev,"A","NATAL");
    alert("Data Published Successfully")
    return false;
}

function UnpublishNatalSignificators(id) {
    var update = id.split('_')
    var update1 = update[1];
    var key = dsexec.Tables[0].Rows[update1].KEY_STRINGS
    mystring = document.getElementById("key_" + update1).value;
    ...
    var res = encyclopaedia.PublishUnpublishSignificators(key, key1,housev,"P","NATAL");
