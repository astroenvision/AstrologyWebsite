...
function gridcall() {
    ...
    document.getElementById('nop').className = 'drpo_homenewclb';
...
function search1() {
    ...
    var query = r + ' Rashi in ' + ch + ' in ' + document.getElementById('clgroup').options[document.getElementById('clgroup').selectedIndex].text + ' Group '
    document.getElementById('qry').innerHTML = query;
    sv = sv.slice(0, -1);
    svn = document.getElementById('nop').value;
    var res = research_tool_any.search(rashi1, house1, astrolrger, document.getElementById('clgroup').options[document.getElementById('clgroup').selectedIndex].text, client, sv, svn, vn);
    var ds = res.value;
    var inc = 1;
    var dec = 0;
    ...
    var varga = '';
    var rashi2 = ''
//    var res = research_tool_any.clientinfoall();
//    var ds = res.value;
...
function open_div_button(id) {
    ...
    var res = research_tool_any.clientinfo(varga,astrologer);
    var ds = res.value;
    for (i = 0; i < ds.Tables[0].Rows.length; ++i) {
        if (ds.Tables[0].Rows[i]['PLANET'] != null) {
            cl_na = cl_na + ds.Tables[0].Rows[i]['CLIENT_NAME'] + '<br>';
            cl_ma = cl_ma + ds.Tables[0].Rows[i]['CLIENT_MAIL'] + '<br>';
            rashi2 = rashi2 + ds.Tables[0].Rows[i]['RASHI'] + '<br>';
...
function accountuser() {
    ...
    if (astrologer == 'astrology' || astrologer == 'ASTROLOGY' || astrologer == "") {
        ...
        var grp_cat = 'Natal';
        res = research_tool_any.searchuser(astrologer, grp_cat);
...
function grcli() {
    ...
    if (document.getElementById('clgroup').options[document.getElementById('clgroup').selectedIndex].text == 'General') {
        ...
        res = research_tool_any.searchclient(astrologer, document.getElementById('clgroup').options[document.getElementById('clgroup').selectedIndex].text, 'Natal');
        callback_drp2(res);
    }
    else {
