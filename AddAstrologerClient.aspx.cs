...
public partial class AddAstrologerClient : System.Web.UI.Page
{
    #region Declarations
    ...
    {
        Ajax.Utility.RegisterTypeForAjax(typeof(AddAstrologerClient));
        if (AdminStateManagement == "COOKIE")
        {
            if (Request.Cookies["AdminCookies"] != null)
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    hdnAstrologerIdVal.Value = AdminUserId;
                    ...
                    hdnUserType.Value = "15";
                    hdnRegUserEmailId.Value ="";
                    hdnRegUserId.Value = "";
                    hdncartid.Value ="";
    ...
    {
        //ScriptManager.RegisterStartupScript(this, GetType(), "Close Modal Popup", "cl_dtls();", true);
        ...
        for (Int32 Counter = 0; Counter < grdData.Rows.Count; Counter++)
        {
            ...
            if (chkval.Checked == true)
            {
                ...
                dsSave = obj.SaveAstroReport("INSERT", "0", lblCATEGORYID.Text , AdminUserId , "1" , AdminUserId);
                if (dsSave.Tables[0].Rows.Count > 0)
                {
                    flagcheck = true;
                    string Message = dsSave.Tables[0].Rows[0]["MESSAGE"].ToString();
