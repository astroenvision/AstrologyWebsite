...
public partial class admin_OverallPaidUserList : System.Web.UI.Page
{
    #region Declarations
    admin obj = new admin();
    ...
    protected void BindGrid()
    {
        ...
        ds = obj.GetDashboardDetailsNew("GET_PAID_USER", SearchBy, txtFromDate.Text, txtToDate.Text, ddlSearchBy.SelectedValue, ddlSortColumn.SelectedValue, ddlSortBy.SelectedValue, "2","");
    ...
    protected void ddlSearchBy_SelectChange(object sender, EventArgs e)
    {
        if (ddlSearchBy.SelectedValue == "Date")
        {
            ...
            ToDate.Visible = true;
            Name.Visible = false;
            divPyamentStatus.Visible = false;
        }

        if (ddlSearchBy.SelectedValue == "PaymentStatus")
        {
    ...
    protected void grdData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            DataSet ds = new DataSet();
            ds = sub_obj.AstDeleteCommon(rowno, "", "", "", "DELETE_PAID_USER_REPORT");
            if (ds.Tables[0].Rows.Count > 0)
            {
                string outresult = ds.Tables[0].Rows[0]["OUTRESULT"].ToString();
                if (outresult == "Y")
                {
                    Response.Redirect("~/admin/OverallPaidUserList.aspx");
                    //BindGrid();
                    //ScriptManager.RegisterClientScriptBlock(this, typeof(admin_FreeUserList), "wq", "alert('Record deleted successfully!');", true);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, typeof(admin_OverallPaidUserList), "wq", "alert('Some error occured!');", true);
                    return;
                }
