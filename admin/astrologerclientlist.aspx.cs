...
public partial class admin_astrologerclientlist : System.Web.UI.Page
{
    #region Declarations
    admin obj = new admin();
        ...
        if (AdminStateManagement == "COOKIE")
        {
            if (Request.Cookies["AdminCookies"] != null)
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    {
                        UpdateClientID(Request.QueryString["clientid"].ToString());
                    }
                }
            }
            else
    ...
    #region UpdateClientID
    protected void UpdateClientID(string ClientID)
    {
        DataSet ds = new DataSet();
        ds = obj.SaveAstroReport("UPDATE", ClientID, "0", "0", "", "");
        if (ds.Tables[0].Rows.Count > 0)
        {
            Response.Redirect("~/admin/astrologerclientlist.aspx");
        }
    }
    #endregion
    ...
    protected void BindGrid()
    {
        ...
        if (Request.Cookies["AdminCookies"]["IsAdmin"].ToString() != null)
        {
            ...
            ds = obj.GetClientList("GET_CLIENT_LIST", txtUserName.Text, txtFromDate.Text, txtToDate.Text, AstrologerID);
        if (ds.Tables[0].Rows.Count > 0)
        {
