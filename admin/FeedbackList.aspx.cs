...
public partial class admin_FeedbackList : System.Web.UI.Page
{
    ...
    protected void BindGrid()
    {
        ...
        ds = obj.GetDashboardDetailsNew("GET_FEEDBACK", txtUserName.Text,"","","" ,ddlSortColumn.SelectedValue , ddlSortBy.SelectedValue,"", Month);
    ...
    protected void grdData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            DataSet ds = new DataSet();
            ds = sub.SaveFeedBack("Delete_FeedBack", rowno, "", "", "", "");
            if (ds.Tables[0].Rows.Count > 0)
            {
                string outresult = ds.Tables[0].Rows[0]["STATUS"].ToString();
                if (outresult == "1")
                {
                    Response.Redirect("~/admin/FeedbackList.aspx");
                    //BindGrid();
                    //ScriptManager.RegisterClientScriptBlock(this, typeof(admin_FreeUserList), "wq", "alert('Record deleted successfully!');", true);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, typeof(admin_FeedbackList), "wq", "alert('Some error occured!');", true);
                    return;
                }
