...
public partial class admin_EditArticlesRequest : System.Web.UI.Page
{
    #region Declaration
    public string ID = "0";
    ...
    protected void LoadData(string NewID)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                txtSynopsis.Text = ds.Tables[0].Rows[0]["SYNOPSIS"].ToString();
            }
           
            if (ds.Tables[0].Rows[0]["FULLSTORY"].ToString() != "")
            {
                txtFullStory.Text = ds.Tables[0].Rows[0]["FULLSTORY"].ToString();
            }
    ...
    {
        DataSet ds = new DataSet();
        ds = obj.Update_Article(txtGroupName.Text.Trim(), lblCatID.Text, txtHeadLine.Text.Trim(), txtSynopsis.Text.Trim(), txtFullStory.Text.Trim(), 
            txtName.Text.Trim(), imgauthor.ImageUrl, lblStatus.Text, ddlPriority.SelectedValue, "", ID,"","","","");
        if(ds.Tables[0].Rows.Count>0)
        {
            if(ds.Tables[0].Rows[0]["FLAG"].ToString() == "Y")
            {
                ScriptManager.RegisterClientScriptBlock(this, typeof(admin_EditArticlesRequest), "wq", "alert('Records Updated Sucessfully!!');", true);
                //Response.AddHeader("REFRESH", "2;URL=/astrology/admin/ApproveArticles.aspx");
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(this, typeof(admin_EditArticlesRequest), "wq", "alert('Some Error Occured!!');", true);
            }
        }
        else
        {
