...
public partial class admin_FreeMatchingUser : System.Web.UI.Page
{
    #region Declarations
    admin obj = new admin();
    ...
    protected void BindGrid()
    {
        ...
        ds = obj.GetDashboardDetailsNew("GET_MATCHING_USER", txtUserName.Text, txtFromDate.Text, txtToDate.Text, ddlSearchBy.SelectedValue, ddlSortColumn.SelectedValue, ddlSortBy.SelectedValue ,"7" , Month);
    ...
    protected void grdData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            DataSet ds = new DataSet();
            ds = sub_obj.AstDeleteCommon(rowno.ToString(), "", "", "", "DELETE_FREE_MATCHING_CHART_DETAILS");
            if (ds.Tables[0].Rows.Count > 0)
            {
                string outresult = ds.Tables[0].Rows[0]["OUTRESULT"].ToString();
                if (outresult == "Y")
                {
                    Response.Redirect("~/admin/FreeUserList.aspx");
                    //BindGrid();
                    //ScriptManager.RegisterClientScriptBlock(this, typeof(admin_FreeUserList), "wq", "alert('Record deleted successfully!');", true);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, typeof(admin_FreeMatchingUser), "wq", "alert('Some error occured!');", true);
                    return;
                }
