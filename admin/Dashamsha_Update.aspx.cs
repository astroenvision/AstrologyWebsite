...
public partial class admin_Dashamsha_Update : System.Web.UI.Page
{
    #region Declarations
    admin obj = new admin();
    ...
    protected void LoadData(string Id)
    {
        ...
        ds = obj.GetAshtakootPredictive("Dashamsha_Predictive", Id, "", "");
        if (ds.Tables[0].Rows.Count > 0)
        {
            txtDeity.Text = ds.Tables[0].Rows[0]["DEITY"].ToString();
            txtDirection.Text = ds.Tables[0].Rows[0]["DIRECTION"].ToString();
            txtRulingPlanet.Text = ds.Tables[0].Rows[0]["RULING_PLANET"].ToString();
    ...
    protected void btnUpdate_click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterClientScriptBlock(this, typeof(admin_Dashamsha_Update), "wq", "alert('Enter Predictive!');", true);
            return;
        }
        ds = obj.UpdateDashamshaPredictive("UPDATE_DASHAMSHA", lblID.Text, txtDeity.Text, txtDirection.Text , txtRulingPlanet.Text , rtePrePredictive.Value 
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            if (STATUS == "1")
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this, typeof(admin_Dashamsha_Update), "wq", "alert(' " + Message + "');", true);
                LoadData(lblID.Text);
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(this, typeof(admin_Dashamsha_Update), "wq", "alert('Some Error Occured');", true);
            }
        }
        else
        {
