...
public partial class admin_PaidUserList : System.Web.UI.Page
{
    #region Declarations
    admin obj = new admin();
    ...
    protected void BindGrid()
    {
        ...
        ds = obj.GetDashboardDetailsNew("GET_PAID_USER", SearchBy, txtFromDate.Text, txtToDate.Text, ddlSearchBy.SelectedValue,ddlSortColumn.SelectedValue,ddlSortBy.SelectedValue , "2" , Convert.ToString(dt.Month));
    ...
    protected void grdData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (outresult == "Y")
                {
                    Response.Redirect("~/admin/PaidUserList.aspx");
                    //BindGrid();
                    //ScriptManager.RegisterClientScriptBlock(this, typeof(admin_FreeUserList), "wq", "alert('Record deleted successfully!');", true);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, typeof(admin_PaidUserList), "wq", "alert('Some error occured!');", true);
                    return;
                }
