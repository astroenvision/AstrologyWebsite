...
public partial class admin_Dosha_Samya_Update : System.Web.UI.Page
{
    #region Declarations
    admin obj = new admin();
    ...
    protected void LoadData(string Id)
    {
        ...
        ds = obj.GetAshtakootPredictive("DoshaSamya_Predictive", Id, "", "");
        if (ds.Tables[0].Rows.Count > 0)
        {
            txtDoshaSamyaBoy.Text = ds.Tables[0].Rows[0]["DOSHA_SAMYA_BOY"].ToString();
            txtDoshaSamyaGirl.Text = ds.Tables[0].Rows[0]["DOSHA_SAMYA_GIRL"].ToString();
            txtPredictiveFor.Text = ds.Tables[0].Rows[0]["PREDICTIVE_FOR"].ToString();
            txtDoshaSamyaFor.Text = ds.Tables[0].Rows[0]["DOSHA_SAMYA_FOR"].ToString();
    ...
    protected void btnUpdate_click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterClientScriptBlock(this, typeof(admin_Dosha_Samya_Update), "wq", "alert('Enter Predictive!');", true);
            return;
        }
        ds = obj.UpdateDoshaSamyaPredictive("UPDATE_DOSHA_SAMYA", lblID.Text, rteQuestion.Value, fckPredictive.Value, fckRemedial.Value,
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            if (STATUS == "1")
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this, typeof(admin_Dosha_Samya_Update), "wq", "alert(' " + Message + "');", true);
                LoadData(lblID.Text);
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(this, typeof(admin_Dosha_Samya_Update), "wq", "alert('Some Error Occured');", true);
            }
        }
        else
        {
