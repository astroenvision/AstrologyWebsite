...
public partial class freeaskquestion : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            Ajax.Utility.RegisterTypeForAjax(typeof(freeaskquestion));
            ...
            if (!IsPostBack)
            {
                if (Session["UserID"] != null)
                {
                    ...
                    txtmail.Text = "free_ask_one_question@gmail.com";
                    txtModileNo.Text = "9999999999";
                ...
                {
                    Session["CurrentPage"] = "FREE_ASK_ONE_QUESTION";
    ...
    [Ajax.AjaxMethod]
    public DataSet SaveClientQuestion(string flag, string QuestionID, string Question, string Answer, string Name, string Dob, string Tob,string Country, string State, string City, string CreatedBy, string RegUserID, string RegUserEmailID,string Status, string IsValid, string Flag1, string Flag2)
    {
        DataSet ds = new DataSet();
        if (ConfigurationManager.AppSettings["ConnectionType"].ToString() == "orcl")
        {
            ASTROLOGY.classesoracle.subscription objsub = new ASTROLOGY.classesoracle.subscription();
            ds = objsub.SaveClientQuestion(flag, QuestionID, Question, Answer, Name, Dob, Tob, Country, State, City, CreatedBy, RegUserID, RegUserEmailID,Status, IsValid, Flag1, Flag2);
        }
        return ds;
    }
