...
public partial class most_important_planet : System.Web.UI.Page
{
    ...
    string titleval = "", keywordsval = "", descval = "";
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            Ajax.Utility.RegisterTypeForAjax(typeof(most_important_planet));
            ...
            if (!IsPostBack)
            {
                ...
                {
                    hdnUserEmailID.Value = Session["UserEmailID"].ToString();
                }
                ...
                if (Request.QueryString["flag"] != null)
                {
                    ...
                    if (flagval== "single")
                    {
                        hdnflagval.Value = flagval;
                        hdnUserType.Value = "9";
                        titleval = "The Most Important planet for You";
                        hdnPaymentFor.Value = "THE_MOST_IMPORTANT_PLANET";
                        heading_div.InnerHtml = titleval;
                        keywordsval = "";
                        descval = "";
                        BindPrice("53");
                    }
                    ...
                    {
                        hdnflagval.Value = flagval;
                        hdnUserType.Value = "10";
                        titleval = "Two Most Important planet for You";
                        hdnPaymentFor.Value = "TWO_IMPORTANT_PLANET";
                        heading_div.InnerHtml = titleval;
                        keywordsval = "";
                        descval = "";
                        BindPrice("54");
                    ...
                    {
                        hdnflagval.Value = flagval;
                        hdnUserType.Value = "11";
                        titleval = "Three Most Important planet for You";
                        hdnPaymentFor.Value = "THREE_IMPORTANT_PLANET;";
                        heading_div.InnerHtml = titleval;
                        keywordsval = "";
                        descval = "";
                        BindPrice("55");
                ...
                if (Session["UserEmailID"] == null)
                {
                    string titleval_redirect = cs.ReplaceQuotes(titleval);
                    titleval_redirect = cs.OptimizeURL(titleval_redirect);
                    Response.Redirect(ResolveUrl("signin.html?flag="+ titleval_redirect + ""));
                }
    ...
    #region Bind Price
    protected void BindPrice(string CatID)
    {
        DataSet ds = new DataSet();
        ds = objsub.AstGetCommon(CatID, "", "", "GET_KUNDLI_MATCHING");
        if (ds.Tables[0].Rows.Count > 0)
        {
            hdnPayableAmount.Value =  ds.Tables[0].Rows[0]["PAYABLE_PRICE"].ToString();
            hdnActualAmount.Value = ds.Tables[0].Rows[0]["ACTUAL_PRICE"].ToString();
            hdnDiscount.Value = ds.Tables[0].Rows[0]["DISCOUNT"].ToString();

            hdnDiscountAmount.Value = Convert.ToString(Math.Round(Convert.ToDouble(Convert.ToDouble(hdnActualAmount.Value) - Convert.ToDouble(hdnPayableAmount.Value)), 2));
